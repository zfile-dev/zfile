package im.zhaojun.zfile.common.controller.onedrive;
...
import im.zhaojun.zfile.home.model.request.sharepoint.SharePointSiteListsRequest;
import im.zhaojun.zfile.admin.model.result.sharepoint.SharepointSite;
import im.zhaojun.zfile.admin.model.result.sharepoint.SharepointSiteList;
import im.zhaojun.zfile.home.model.request.SharePointInfoRequest;
...
public class SharePointHelperController {
    ...
    public AjaxJson<List<SharepointSite>> getSites(@Valid @RequestBody SharePointSearchSitesRequest searchSitesRequest) {
        ...
        for (int i = 0; i < valueArray.size(); i++) {
            SharepointSite sharepointSite = valueArray.getObject(i, SharepointSite.class);
            sites.add(sharepointSite);
        }
    ...
    public AjaxJson<List<SharepointSiteList>> getSites(@Valid @RequestBody SharePointSiteListsRequest sharePointSiteListsRequest) {
        List<SharepointSiteList> sites = new ArrayList<>();
        ...
        JSONArray valueArray = rootObject.getJSONArray("value");
        for (int i = 0; i < valueArray.size(); i++) {
            SharepointSiteList sharepointSiteList = valueArray.getObject(i, SharepointSiteList.class);
            // 如果是事件目录，则跳过
            if (Objects.equals(SHAREPOINT_LIST_TYPE_EVENT, sharepointSiteList.getDisplayName())) {
                continue;
            }

            // 如果是文档类型，则改名为"默认文档"
            if (Objects.equals(SHAREPOINT_LIST_TYPE_DOCUMENT, sharepointSiteList.getDisplayName())) {
                sharepointSiteList.setDisplayName("默认文档");
            }
            sites.add(sharepointSiteList);
        }
        sites.sort(Comparator.comparing(SharepointSiteList::getCreatedDateTime));
        return AjaxJson.getSuccessData(sites);
    }
